...
public class OperatorBuilderExtension extends BuilderExtension<OperatorTemplate> {

    @Override
    public void action(OperatorTemplate object, Deque<TreeType> stack, State state) {
        TreeType right = stack.pop();
        TreeType left = stack.pop();
        stack.push(new Operator(left, right, object.getOperation()));
    }

    @Override
    public boolean accept(Flag flag) {
        return flag.isOperator();
    }
}
